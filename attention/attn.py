...
def llama_flash_attn2_forward(
    ...
    **kwargs: Unpack[FlashAttentionKwargs],
    ...
    if getattr(past_key_value, "get_score", None):  # calculate KV importance
        if getattr(past_key_value, "prefill_mode", None):
            past_key_value._get_score_flatten(query_states, key_states, self.layer_idx)
        else:
            past_key_value._get_score(query_states, key_states, self.layer_idx)

    if getattr(past_key_value, "evicted", None): 
        # after eviction, kv is flattened, so we need to update meta-data in prepare function
        query_states, key_states, value_states, info = past_key_value.prepare( 
            query_states, key_states, value_states, self.layer_idx, prefill_mode=getattr(past_key_value, "prefill_mode", False))
