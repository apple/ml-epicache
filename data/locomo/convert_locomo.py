...
def build_conversation_context(conversation_data: Dict[str, Any]) -> str:
    """
    Build the full conversation context from LocoMo conversation data.
    Similar to get_input_context function from run_locomo.py
    """
    # Get speaker names
    ...
    context = CONV_START_PROMPT.format(speaker_a, speaker_b)
    ...
    for session_num in session_numbers:
        ...
        if session_key in conversation_data and conversation_data[session_key]:
            ...
            if datetime_key in conversation_data:
                conversation_text += f"\nDATE: {conversation_data[datetime_key]}\n"
            ...
            for dialog in conversation_data[session_key]:
                ...
                # Handle blip_caption if present (from run_locomo.py)
...
def convert_locomo_to_scbench(locomo_path: str, output_path: str) -> None:
    """
    Convert LocoMo format to SCBench format
    """
    ...
    for conv_idx, conversation in enumerate(locomo_data):
        print(f"Processing conversation {conv_idx + 1}/{len(locomo_data)}")
        
        # Build conversation context
        conversation_context = build_conversation_context(conversation['conversation'])
        ...
        for qa_idx, qa in enumerate(qa_list):
            # Skip adversarial tasks (those without 'answer' key)
            # if 'answer' not in qa:
            #     total_skipped += 1
            #     continue
            
            if qa['category'] == 5: # we don't use task type 5 for now (adversarial)
                continue
            ...
            # elif category == 5:
            #     question = qa['question']
            #     question = question + " (a) {} (b) {}. Select the correct answer by writing (a) or (b)."
        ...
        if len(ground_truth) > 0:
            ...
            scbench_entry = {
                ...
                'options': []  # LocoMo doesn't have multiple choice options
            }
    ...
    print(f"- Skipped adversarial tasks: {total_skipped}")
...
def main():
    ...
    output_path = "locomo_preprocessed.pt"
    
    # Convert the data
    convert_locomo_to_scbench(locomo_path, output_path)
